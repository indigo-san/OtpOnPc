...
public sealed class ListBoxItemBehavior : Behavior<ListBoxItem>
{
    ...
    private void Released(object? sender, PointerReleasedEventArgs e)
    {
        if (Equals(e.Pointer.Captured, AssociatedObject))
        {
            if (e.InitialPressMouseButton == MouseButton.Left)
            {
                if (_doubleClick)
                {
                    if (AssociatedObject?.DataContext is TotpItemViewModel totpItemViewModel
                        && _itemsControl?.DataContext is MainPageViewModel viewModel)
                    {
                        viewModel.CopySelected.Execute(totpItemViewModel);
                    }
                }
                else
                {
    ...
    private static async void MoveDraggedItem(ItemsControl? itemsControl, int oldIndex, int newIndex)
    {
        if (itemsControl?.DataContext is MainPageViewModel viewModel)
        {
            await viewModel.MoveItem(oldIndex, newIndex);
        }
    ...
    private void Moved(object? sender, PointerEventArgs e)
    {
        ...
        {
            if (_itemsControl?.Items is null || AssociatedObject?.RenderTransform is null || !_enableDrag)
            {
                return;
            }
            ...
            _draggedIndex = _itemsControl.ItemContainerGenerator.IndexFromContainer(AssociatedObject);
            _targetIndex = -1;
